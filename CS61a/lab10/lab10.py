...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == "and":  # and expressions
            ...
            op_func = calc_eval(operator)
            evaluated_operands = operands.map(lambda operand: calc_eval(operand))
            return calc_apply(
                op_func, evaluated_operands
...
def eval_and(expressions):
    ...
    while expressions is not nil:
        ...
        if cur_value is scheme_f or expressions.rest is nil:
            return cur_value
