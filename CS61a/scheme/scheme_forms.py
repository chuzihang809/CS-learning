...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        function_symbol = signature.first
        formals = signature.rest
        body = expressions.rest
        lambda_function = do_lambda_form(Pair(formals, body), env)
        env.define(function_symbol, lambda_function)
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        cur_value = scheme_eval(expressions.first, env)
        if is_scheme_false(cur_value) or expressions.rest is nil:
            return cur_value
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        cur_value = scheme_eval(expressions.first, env)
        if is_scheme_true(cur_value) or expressions.rest is nil:
            return cur_value
        expressions = expressions.rest
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        names, vals = Pair(pair.first, names), Pair(value, vals)
