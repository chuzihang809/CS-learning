def shuffle(s):
    ...
    for i in range(length // 2):
        new_list = new_list + [s[: (length // 2)][i]] + [s[(length // 2) :][i]]
    return new_list
...
def balanced(m):
    ...
    def torque(m):
        return length(left(m)) * total_mass(end(left(m))) == length(
            right(m)
        ) * total_mass(end(right(m)))

    if is_planet(m):
        return True
    else:
        return torque(m) and balanced(end(left(m))) and balanced(end(right(m)))
...
def berry_finder(t):
    ...
    if label(t) == "berry":
        ...
        lists = [berry_finder(b) for b in branches(t)]
        for i in lists:
            if i == True:
                return True
        return False
